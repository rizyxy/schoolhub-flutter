...
class TopicBlocConsumer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<TopicBloc, TopicState>(listener: (context, state) {
      if (state is TopicError) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(state.errorMessage ??
                "An error has occured when fetching topics")));
      }
      ...
      if (state is TopicError) {
        return Column(
          ...
          children: <Widget>[
            InkWell(
              onTap: () {
                context.read<TopicBloc>().add(FetchTopics(subject: subject));
              },
